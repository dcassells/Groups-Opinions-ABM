...
import logging
logging.basicConfig(filename='model_runner_R.log', encoding='utf-8', level=logging.DEBUG)
...
##########
# run simulations for R
##########
vals_a = [0.01,0.1 ,0.4 ,0.01,0.1 ,0.4 ,0.01,0.1 ,0.4 ]
vals_b = [0.01,0.01,0.01,0.05,0.05,0.05,0.25,0.25,0.25]
...
def val_selection(T_vals,E_vals,g_methods):
    for T, E, g_method in zip(T_vals,E_vals,g_methods):

        fname = 'sim_data/'+g_method+'_R_sim_T'+str(T)+'_E'+str(E)+'.pkl'
        ...
        df_new = run_experiments(pop_size=pop_size, opinion_dims=opinion_dims, num_clusters=num_clusters,
                               graph_type=graph_type, T_in=T, T_out=T, E_in=E, E_out=E, group_dependent_param='R', group_method=g_method,
