...
import logging
logging.basicConfig(filename='model_runner_E.log', encoding='utf-8', level=logging.DEBUG)
...
##########
# run simulations for E
##########
vals_a = [0.01,0.01,0.01,0.05,0.05,0.05,0.25,0.25,0.25]
vals_b = [0.01,0.1 ,0.4 ,0.01,0.1 ,0.4 ,0.01,0.1 ,0.4 ]
...
def val_selection(R_vals,T_vals,g_methods):
    for R, T, g_method in zip(R_vals,T_vals,g_methods):

        fname = 'sim_data/'+g_method+'_E_sim_R'+str(R)+'_T'+str(T)+'.pkl'
        ...
        df_new = run_experiments(pop_size=pop_size, opinion_dims=opinion_dims, num_clusters=num_clusters,
                               graph_type=graph_type, R_in=R, R_out=R, T_in=T, T_out=T, group_dependent_param='E', group_method=g_method,
